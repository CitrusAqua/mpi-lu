...
void row_node(int rank)
{
    ...
    if(rank==0)
    {
        ...
        float kk[4] __attribute__((aligned(16)))={0,0,0,0};
        __m128* t2=(__m128*)kk;
        for(int k=0; k < r_n; k++)
        {
            // division
            ...
            for(int j=k+1;j<N;j=j+4)
            {
                ...
                if(j%4==0)
                {
                    __m128* t1 = (__m128*)&A[k][j];
                    *t1=_mm_div_ps(*t1,*t2);
                }
                else
                {
                    for(int extra=j;extra<j-(j%4)+4;extra++)
                    {
            ...
            for(int i=k+1;i<r_n;i++)
            {
                ...
                for(int j=k+1;j<N;j=j+4)
                {
                    if(j%4==0)
                    {
                        __m128* tij=(__m128*)&A[i][j];
                        __m128* tkj=(__m128*)&A[k][j];
                        __m128 mul_result;
                        mul_result=_mm_mul_ps(*t2,*tkj);
                        *tij=_mm_sub_ps(*tij,mul_result);
                    }
                    else{
                        for(int extra=j;extra<j-(j%4)+4;extra++)
                        {
    ...
    else
    {
        ...
        */
        ...
        for(int i=0; i<block_size; i++)
            block[i] = (float*)_mm_malloc(sizeof(float)*N,16);
        //接收初始值
        //cout<<"yes3"<<endl;
        for(int i=0;i<block_size;i++)
        {
        ...
        float *rrow=(float*)_mm_malloc(sizeof(float)*N,16);
        //接收数据
        float kk[4] __attribute__((aligned(16)))={0,0,0,0};
        __m128* t2=(__m128*)kk;
            ...
            for(int i=0; i<block_size; i++)
            {
                ...
                */
                for(int j=k+1; j<N; j=j+4)
                {
                    if(j%4==0)
                    {
                        __m128* tij=(__m128*)&block[i][j];
                        __m128* tkj=(__m128*)&rrow[j];
                        __m128 mul_result;
                        mul_result=_mm_mul_ps(*t2,*tkj);
                        *tij=_mm_sub_ps(*tij,mul_result);
                    }
        ...
        for(int k=begin;k<=end;k++)
        {
            ...
            */
            for(int j=k+1;j<N;j=j+4)
            {
                ...
                if(j%4==0)
                {
                    __m128* t1 = (__m128*)&block[k-begin][j];
                    *t1=_mm_div_ps(*t1,*t2);
                }
                else
                {
            ...
            for(int true_i=i+1; true_i<block_size; true_i++)
            {
                ...
                */
                for(int j=k+1;j<N;j=j+4)
                {
                    if(j%4==0)
                    {
                        __m128* tij=(__m128*)&block[true_i][j];
                        __m128* tkj=(__m128*)&block[i][j];
                        __m128 mul_result;
                        mul_result=_mm_mul_ps(*t2,*tkj);
                        *tij=_mm_sub_ps(*tij,mul_result);
                    }
